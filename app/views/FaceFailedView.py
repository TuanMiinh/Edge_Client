...
BACKGROUND_PATH = "assets/images/bg_denied.png"
BTN_RETRY_PATH = "assets/button/btn.retry.png"
BTN_CONFIRM_PATH = "assets/button/btn.confirm_2.png"
BTN_CONFIRM2_PATH = "assets/button/btn-confirm_1.png"
BTN_RETRY2_PATH = "assets/button/btn.retry (1).png"

class FaceFailedView(tk.Frame):
    def __init__(self, parent, controller):
        tk.Frame.__init__(self, parent, bg='white')
        self.controller = controller
        self.master = parent
        print(datetime.now().strftime("%H:%M:%S") + ": Init FaceFailed view ...")
    ...
    def load_image(self):
        ...
        with Image.open(BTN_RETRY_PATH) as img:
            self.img_btn_retry = img.copy()
            self.imgtk_btn_retry = ImageTk.PhotoImage(self.img_btn_retry)

            self.img_btn_retry_resize = img.resize((130,50)).copy()
            self.imgtk_btn_retry_resize = ImageTk.PhotoImage(self.img_btn_retry_resize)
        ...
        with Image.open(BTN_RETRY2_PATH).resize((130,50)) as img:
            self.img_btn_retry_success = img.copy()
            self.imgtk_btn_retry_success = ImageTk.PhotoImage(self.img_btn_retry_success)
        with Image.open(BTN_CONFIRM2_PATH).resize((150,50)) as img:
            self.img_btn_confirm_success = img.copy()
            self.imgtk_btn_confirm_success = ImageTk.PhotoImage(self.img_btn_confirm_success)
    ...
    def create_widgets(self):
        ...
        self.text = tk.Label(
            self.left_frame, text="", font=('Courier New', 22, 'bold'), background=DANGER_COLOR, fg=TEXT_WHITE_COLOR)
        self.text.place(relx=0.5, rely=0.8, anchor="center", relwidth=0.5)

        # btn
        self.btnRetry = tk.Button(
            self.left_frame, bd=0, highlightthickness=0, image=self.imgtk_btn_retry,
            background="#6d0a0a", activebackground="#6d0a0a",
            command=lambda: self.controller.change_state(RecognizeState.RETRY))
        self.btnRetry.place(relx=0.27, rely=0.9, anchor="nw")
    ...
    def show_popup(self, command: str, user_info: dict = None):
        if command == "FOUND":
            ...
            btn_confirm = tk.Button(form_info, bd=0, highlightthickness=0, image=self.imgtk_btn_confirm_success,
                                    background="#0f5d54", activebackground="#0f5d54",
                                    command=lambda: self.controller.change_state(RecognizeState.RECOGNIZED_SUCESSED))
            btn_confirm.place(relx=0.15, rely=0.75)

            btn_retry = tk.Button(form_info, bd=0, highlightthickness=0, image=self.imgtk_btn_retry_success,
            ...
            btn_retry = tk.Button(popup, bd=0, highlightthickness=0, image=self.imgtk_btn_retry_resize,
    ...
    def retry(self):
        ...
        if self.controller.RecognizeState == RecognizeState.RECOGNIZING:
            self.left_frame_bg.after(50, self.retry)
